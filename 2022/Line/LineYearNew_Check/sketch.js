...
function setup() {
  ...
  for (let i = 0; i < numRows; i++) {
    ...
    if (year == jahr) {
      let unterteilung = (width - 2 * paddinglinksrechts) / 365;
      ...
      quarterHours = map(quarterHours, 0, 96, 0, unterteilung);
      ...
      valueArray.push([posX, posYB, posYZ, dateString, weekdayRaw]);
    }
  }
  //
}
...
function drawSkala() {
  ...
  textAlign(CENTER, CENTER);
  text("Stromverbrauch für das Jahr", paddinglinksrechts, abstandobenunten);
