...
let ratio = 1.77;
let ratioPaddinglinksrechts = 10;
...
function setup() {
  ...
  for (let i = 0; i < numRows; i++) {
    ...
    let einAbschnitt = längeGrafik / 14;
...
function drawSkala() {
  ...
  for (let i = 0; i <= 11; i++) {
    ...
    if (i % 2 == 0 || i == 0) {
      ...
      text(
        zahlenSkala[i],
        width - paddinglinksrechts + abstandSkala,
        skalaPosY
      );
    }
  }
  ...
  textSize(textgrösseZBKWH);
  fill(schriftFarbe);
  text("MWh", width - abstandSkala, abstandTag);
  text("Zürich", zürichbaselabstand, (positions[4] + positions[6]) / 2);
