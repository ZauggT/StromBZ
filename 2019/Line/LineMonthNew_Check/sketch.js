...
function keyPressed() {
  if (key == "s") {
    saveCanvas("Line_month_2022", "png");
  }
}
...
function setup() {
  ...
  for (let i = 0; i < numRows; i++) {
    ...
    let padding = 250;
    let einAbschnitt = (-2 * paddinglinksrechts + width) / 12;
    let posYB;
    let posYZ;
    let posX;
    if (year == jahr) {
      ...
      quarterHours = map(quarterHours, 0, 96, 0, einAbschnitt);
